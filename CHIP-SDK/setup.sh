...
device-tree-compiler libncurses-dev ppp cu linux-image-extra-virtual \
u-boot-tools android-tools-fastboot android-tools-fsutils python-dev python-pip \
libusb-1.0-0-dev g++-arm-linux-gnueabihf pkg-config libacl1-dev zlib1g-dev \
liblzo2-dev uuid-dev

if uname -a |grep -q 64;
then
  echo -e "\n Installing 32bit compatibility libraries"
  sudo apt -y install libc6-i386 lib32stdc++6 lib32z1
fi
